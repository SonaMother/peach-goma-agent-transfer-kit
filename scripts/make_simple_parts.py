#!/usr/bin/env python3
"""make_simple_parts.py - the "simple parts" channel (user-requested v4 design).

Splits every tracked source file into tiny plain-text part files:
...
- caps chosen from measured data: avg 38.7 B/line -> 100 lines ~ 3.8 KB
  => LINE_CAP=100 and BYTE_CAP=4000 (final part file, header included)
- each part: line 1 = "[PART k of n | path]" header, then exact content
- filenames: p001__src__character__Cat.tsx__part03of14.txt (self-describing)
- parts are numbered globally so the agent just walks p001 -> pNNN
- package-lock.json excluded: npm install regenerates it
- PARTS_INDEX.md: self-calibration probes + 6-step read/write loop + table
"""
import subprocess
...
EXCLUDE_FILES = {
    "package-lock.json",            # regenerable via npm install
    "gameproject.zip", "RECREATE_PROJECT.md", "AGENT_PROMPT.txt",
    "PARTS_INDEX.md",
}
EXCLUDE_PREFIXES = ("bundle/", "parts/", "parts-esc/")

RAW_DIR = REPO / "parts"
ESC_DIR = REPO / "parts-esc"
...
def build_set(files: list[str], out_dir: Path, escape: bool):
    ...
    for rel in files:
        ...
        rows.append((rel, len((REPO / rel).read_bytes()),
                     first_seq, seq, n))
    return rows, seq


def index_md(rows_raw, rows_esc, n_raw, n_esc) -> str:
    ...
    L.append("1. Run npm install once now. It regenerates package-lock.json, which")
    L.append("   is intentionally not shipped here.")
    L.append("2. Process parts strictly in order: p001, p002, p003, ... (one")
    L.append("   fetch per part). Part filenames follow one rule:")
    L.append("   pNNN__<file path, with / written as __>__partKKofNN.txt")
    L.append(f"   Example: {ex_name} holds the start of {ex_rel}.")
    L.append("   Cannot list a folder? https://ungh.cc/repos/SonaMother/peach-goma-skyhop-3d-cat-platformer/files/main")
    ...
    L.append("   https://cdn.jsdelivr.net/gh/SonaMother/peach-goma-skyhop-3d-cat-platformer@main/parts/FILE.txt")
    ...
    L.append("   Never hold whole files in memory; never rewrite from memory.")
    ...
    L.append("7. After the last part: npm run dev. Done.\n")
    ...
    L.append("- src/main.tsx is 230 bytes and starts with: import { StrictMode }")
    L.append("- index.html is 783 bytes and references /src/main.tsx (Vite, not CRA;")
    L.append("  any %PUBLIC_URL% means you fabricated it - redo that file)")
    L.append("- src/character/Cat.tsx is 58,242 bytes, the largest file")
    L.append("- The game builds with: npm install && npm run dev")
    return "\n".join(L) + "\n"
...
def main():
    ...
    rows_raw, n_raw = build_set(tracked, RAW_DIR, escape=False)
    rows_esc, n_esc = build_set(tracked, ESC_DIR, escape=True)
    assert [r[0] for r in rows_raw] == [r[0] for r in rows_esc]

    (REPO / "PARTS_INDEX.md").write_text(
        index_md(rows_raw, rows_esc, n_raw, n_esc), encoding="utf-8")
    ...
    biggest_raw = max((RAW_DIR / f).stat().st_size for f in
                      [p.name for p in RAW_DIR.glob('*.txt')])
    biggest_esc = max((ESC_DIR / f).stat().st_size for f in
                      [p.name for p in ESC_DIR.glob('*.txt')])
